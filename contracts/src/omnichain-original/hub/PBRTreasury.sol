 ...
 * @notice Receives ETH fees from SETH transfers and distributes Performance Based Returns to verified applications
 */
contract PBRTreasury is OApp, FunctionsClient, ReentrancyGuard {
    ...
    constructor(address _pbrManager, address _appRegistry, address _protocol) {
        if (_pbrManager == address(0) || _appRegistry == address(0) || _protocol == address(0)) revert InvalidAddress();
    ...
    function getData() external onlyManager {
        - Can be called to periodically keep an up-to-date list of verifiedTvl values (hourly automation with local storage)
        - Keeps 24hrs worth of data on a rolling basis for time-weighted TVL distribution
    ...
    function commitScalar(uint8 cycle) external onlyManager {
        ...
        // - Probably need cooldown logic (6hrs) to rate limit the scalars, then we can allow anyone 
        // to call the function & also automate it with daily sweeps
    ...
    /// @notice Allows a deployer/beneficiary on hubChain to claim PBR
    function claimPbr() external nonReentrant {
        
    }

    /// @notice Allows a deployer/beneficiary on spokeChain to claim PBR
    function claimCrosschain() external nonReentrant {
